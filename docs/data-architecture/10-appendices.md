...
**UX (2026-05 revision)**: the Settings revocation surface was restructured into a single-row pattern — emoji + title + small (i) info button + Switch — with the per-purpose description tucked behind the (i) disclosure (only one row expands at a time, controlled by `expandedConsent` state). Lazy disclosure reduces consent fatigue on the dense Data & Privacy card while preserving Art. 12 transparency for users who tap to read each purpose verbatim. The `documents` toggle label was also narrowed from `'📄 Document & menu analysis'` / `'📄 Análisis de documentos y menús'` to `'📄 Document analysis'` / `'📄 Análisis de documentos'` — school-menu parsing is a sub-use of the same processing purpose, not a separate consent.
...
**Evidence**: `src/modules/consent/ConsentContext.tsx`, `app/onboarding.tsx` (consent step + parental gate), `app/settings.tsx` (revocation UI + collapsible info rows), `src/i18n/{en,es}.ts` (toggle labels + per-purpose descriptions), `src/components/layout/AIFloatingButton.tsx` (runtime gating).
